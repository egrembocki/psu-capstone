...
## Excluding Directories from SonarQube Analysis

To ignore a directory (such as `test/` or `archive/`) in SonarQube analysis, add an exclusion pattern to your SonarQube configuration:

- In your `sonar-project.properties` file, add:

	```
	sonar.exclusions=archive/**/*,test/**/*
	```

- You can also set this property in the SonarQube UI under project settings, or in your CI/CD pipeline configuration.

This will prevent SonarQube from analyzing files in the specified directories.
